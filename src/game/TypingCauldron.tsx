...
export function TypingCauldron() {
  ...
  return <div ref={rootRef} tabIndex={0} onPointerDown={() => rootRef.current?.focus()} className={`min-h-dvh bg-bg text-ink outline-none ${showCreateUser ? "is-creating-user" : ""}`}>
    ...
    {phase === "menu" && <div className="game-overlay"><div className="game-dialog game-dialog--rank"><p className="text-xs font-semibold tracking-[0.18em] text-muted uppercase">Typing challenge</p><h2 className="mt-3 text-4xl font-semibold tracking-tight text-ink">Word by word.</h2><p className="mt-4 max-w-md text-base leading-7 text-muted">Complete the story before 07:00. Every character releases a grain while the container stays alive in the background.</p><label className="mt-6 block text-sm font-medium text-ink">Ranking name<input value={username} maxLength={24} onChange={(event) => setUsername(event.target.value)} placeholder="Enter your name" className="mt-2 h-11 w-full rounded-xl border border-border bg-surface-raised px-3 text-ink outline-none focus:border-accent" /></label><button type="button" disabled={username.trim().length < 2} onClick={() => void start()} className="mt-6 min-h-12 rounded-xl bg-accent px-6 text-sm font-semibold text-bg transition-[transform,opacity] duration-150 hover:opacity-90 active:scale-95 disabled:cursor-not-allowed disabled:opacity-50">Start writing</button><div className="mt-8 border-t border-border pt-5"><p className="text-xs font-semibold tracking-[0.18em] text-muted uppercase">Global ranking</p>{!rankLoaded ? <p className="mt-2 text-sm text-muted">Loading scores...</p> : globalRank.length === 0 ? <p className="mt-2 text-sm text-muted">No scores yet.</p> : <ol className="mt-2 space-y-1 text-sm text-muted">{globalRank.map((score, index) => <li key={`${score.username}-${index}`} className="flex justify-between"><span>#{index + 1} {score.username}</span><span className="font-mono text-ink">{score.words}</span></li>)}</ol>}</div></div></div>}
